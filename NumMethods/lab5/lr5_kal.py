...
def explicit_scheme(Nx, Nt):
    ...
    if r > 0.5:
        print(f"  Явная схема: r = {r:.3f} > 0.5 - может быть неустойчива")
...
def implicit_scheme(Nx, Nt):
    ...
    for k in range(0, Nt-1):
        ...
        b[0] += r * left_boundary(t[k+1])
        b[-1] += r * right_boundary(t[k+1])
...
def crank_nicolson_scheme(Nx, Nt):
    ...
    main_diag_A = np.ones(Nx-2) * (1 + 2*r)
    off_diag_A = np.ones(Nx-3) * (-r)
    A = diags([off_diag_A, main_diag_A, off_diag_A], [-1, 0, 1], format='csc')
    
    main_diag_B = np.ones(Nx-2) * (1 - 2*r)
    off_diag_B = np.ones(Nx-3) * r
    B = diags([off_diag_B, main_diag_B, off_diag_B], [-1, 0, 1], format='csc')
    
    for n in range(0, Nt-1):
        b = B.dot(u[n, 1:-1])
        
        b[0] += r * (left_boundary(t[n+1]) + left_boundary(t[n]))
        b[-1] += r * (right_boundary(t[n+1]) + right_boundary(t[n]))
