...
plt.plot(xs, true_ys, label='Точное решение', color='blue', linewidth=2)
plt.plot(xs, ys, label='Метод конечных разностей', color='red', linestyle='--', marker='o')

plt.xlabel('x')
plt.ylabel('y')
plt.title('Сравнение точного решения и численного (метод конечных разностей)')
plt.legend()
plt.grid(True)
# plt.show()
plt.savefig('4_2_don')