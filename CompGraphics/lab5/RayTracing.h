...
public:
    ...
    Scene() {
        ...
        Material morematteMaterial(Vector3(0.0f, 0.0f, 1.0f), 0.1f, 0.9f, 20);  // 20 выборок для лучей
        Material matteMaterial(Vector3(1.0f, 0.0f, 0.0f), 0.1f, 0.3f, 10);  // 10 выборок для лучей
        Material shinyMaterial(Vector3(0.54f, 0.0f, 1.0f), 0.1f, 0.0f, 40);  // 50 выборок для более детализированного освещения 

        // Добавляем 3 сферы
        spheres.push_back(Sphere(Vector3(0, -1, 5), 1.0f, matteMaterial));
