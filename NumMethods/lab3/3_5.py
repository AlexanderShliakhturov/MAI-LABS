...
#делим ось икс с шагом h, находим сумму произведений значений функции в середине шага на значение аргумента (формула 3.23)
def rectangle(a, b, h):
    total = 0
...
# Метод трапеций
#Заменяем функцию многочленом Лагранжа первой степени, считая узлами интерполяции начало и конец отрезка (формула 3.25)
...
# Метод Симпсона
# Заменяем подынтегральную кривую интерполяционным многочленом второй степени, считая узлами конец и середину отрезка, за hi берем середину отрезка
